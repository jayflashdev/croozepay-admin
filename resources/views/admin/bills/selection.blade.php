...
@php
$providers = [
    'adex1', 'adex2', 'adex3', 'adex4', 'adex5', 'adex6',
    'msorg1', 'msorg2', 'msorg3', 'msorg4', 'msorg5', 'msorg6',
    'ncwallet', 'vtpass', 'clubkonnect', 'easyaccess'
];
    $providers2 = [
        'adex1', 'adex2', 'adex3', 'adex4', 'adex5', 'adex6',
        'msorg1', 'msorg2', 'msorg3', 'msorg4', 'msorg5', 'msorg6',
        'ncwallet', 'vtpass', 'clubkonnect'
    ];
    $providers3 = [
        'adex1', 'adex2', 'adex3', 'adex4', 'adex5', 'adex6',
        'ncwallet',
        ...
        <form action="{{route('admin.setting.api_settings')}}" class="row ajaxForm" method="post">
            ...
            <div class="col-md-4 form-group">
                ...
                <select class="form-select" name="betting_sel" required>
                    <option @if(api_setting('betting_sel') == "clubkonnect")selected @endif value="clubkonnect">Clubkonnect</option>
                </select>
            </div>
            <div class="col-md-4 form-group">
                <input type="hidden" name="types[]" value="bulksms_sel">
                <label class="form-label text-uppercase">Bulksms API</label>
                <select class="form-select" name="bulksms_sel" required>
                    @foreach ($providers3 as $provider)
                        <option value="{{ $provider }}" @if (api_setting('bulksms_sel') == $provider) selected @endif>
                            {{ ucfirst($provider) }}
                        </option>
                    @endforeach
                </select>
        ...
        <form action="{{route('admin.setting.api_settings')}}" class="row ajaxForm" method="post">
            ...
            <div class="col-md-3 form-group">
                ...
                <select class="form-select" name="mtn_datacard" required>
                    @foreach ($providers4 as $provider)
                        <option value="{{ $provider }}" @if (api_setting('mtn_datacard') == $provider) selected @endif>
                            {{ ucfirst($provider) }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="col-md-3 form-group">
                ...
                <select class="form-select" name="glo_datacard" required>
                    @foreach ($providers4 as $provider)
                        <option value="{{ $provider }}" @if (api_setting('glo_datacard') == $provider) selected @endif>
                            {{ ucfirst($provider) }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="col-md-3 form-group">
                ...
                <select class="form-select" name="airtel_datacard" required>
                    @foreach ($providers4 as $provider)
                        <option value="{{ $provider }}" @if (api_setting('airtel_datacard') == $provider) selected @endif>
                            {{ ucfirst($provider) }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="col-md-3 form-group">
                ...
                <select class="form-select" name="mob_datacard" required>
                    @foreach ($providers4 as $provider)
                        <option value="{{ $provider }}" @if (api_setting('mob_datacard') == $provider) selected @endif>
                            {{ ucfirst($provider) }}
                        </option>
                    @endforeach
                </select>
...
<div class="card">
    <h5 class="card-header text-center mb-2">RECHARGECARD API</h5>
    <div class="card-body">
        <form action="{{route('admin.setting.api_settings')}}" class="row ajaxForm" method="post">
            @csrf
            <div class="col-md-3 form-group">
                <input type="hidden" name="types[]" value="mtn_rechargecard">
                <label class="form-label">MTN Rechargecard</label>
                <select class="form-select" name="mtn_rechargecard" required>
                    @foreach ($providers5 as $provider)
                        <option value="{{ $provider }}" @if (api_setting('mtn_rechargecard') == $provider) selected @endif>
                            {{ ucfirst($provider) }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="col-md-3 form-group">
                <input type="hidden" name="types[]" value="glo_rechargecard">
                <label class="form-label">GLO Rechargecard</label>
                <select class="form-select" name="glo_rechargecard" required>
                    @foreach ($providers5 as $provider)
                        <option value="{{ $provider }}" @if (api_setting('glo_rechargecard') == $provider) selected @endif>
                            {{ ucfirst($provider) }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="col-md-3 form-group">
                <input type="hidden" name="types[]" value="airtel_rechargecard">
                <label class="form-label">Airtel Rechargecard</label>
                <select class="form-select" name="airtel_rechargecard" required>
                    @foreach ($providers5 as $provider)
                        <option value="{{ $provider }}" @if (api_setting('airtel_rechargecard') == $provider) selected @endif>
                            {{ ucfirst($provider) }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="col-md-3 form-group">
                <input type="hidden" name="types[]" value="mob_rechargecard">
                <label class="form-label">9Mobile Rechargecard</label>
                <select class="form-select" name="mob_rechargecard" required>
                    @foreach ($providers5 as $provider)
                        <option value="{{ $provider }}" @if (api_setting('mob_rechargecard') == $provider) selected @endif>
                            {{ ucfirst($provider) }}
                        </option>
                    @endforeach
                </select>
