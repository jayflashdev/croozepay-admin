...
@section('title', 'Cable Subscriptions')
          ...
          <tbody>
            @foreach ($trx as $key => $item)
              <tr>
                ...
                <td> {!! transactionStatus($item->status) !!}</td>
                <td>{{$item->code}}
                    <p> {{show_datetime1($item->created_at)}}</p>
                </td>
                <td> {{($item->number)}} </br>
                    {{$item->meta->decoder ?? "Null"}} <br>
                    {{$item->meta->plan ?? "Null"}}
                </td>
                <td>{{format_price($item->amount)}}</td>
                <td>
                    <p>New: </b> {{format_price($item->new_balance)}} </p>
                    <p>Old: </b> {{format_price($item->old_balance)}} </p>
                </td>
                <td>
                    <p>{{$item->system}}</p>
                    {{$item->api_name ?? "none"}}
