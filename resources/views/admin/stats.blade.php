            ...
            <div class="card text-center">
                <div class="card-body">
                    ...
                    <p class="text-muted mb-0 mt-3">
                        <b>{{ App\Models\User::whereDate('created_at', today())->where('blocked', 0)->where('user_role', 'user')->count() }}</b>
                        Today
                    </p>
                </div>
            </div>
        ...
        <div class="col-sm-6 col-lg-3">
            <div class="card text-center">
                <div class="card-body p-t-10">
                    ...
                    <p class="text-muted mb-0 mt-3">
                        <b>{{ format_price(App\Models\Mdeposit::whereDate('updated_at', today())->where('status', 1)->sum('amount')) }}</b>
                        Today
                    </p>
                </div>
            </div>
        ...
        <div class="col-sm-6 col-lg-3">
            <div class="card text-center">
                <div class="card-body p-t-10">
                    ...
                    <p class="text-muted mb-0 mt-3">
                        <b>{{ format_price(App\Models\Deposit::whereDate('updated_at', today())->where('status', 1)->sum('amount')) }}</b>
                        Today
                    </p>
                </div>
            </div>
        ...
        <div class="col-sm-6 col-lg-3">
            <div class="card text-center">
                <div class="card-body p-t-10">
                    ...
                    <p class="text-muted mb-0 mt-3">
                        <b>{{ format_price(App\Models\Mdeposit::whereDate('updated_at', today())->where('status', 2)->sum('amount')) }}</b>
                        Today
                    </p>
                </div>
            </div>
            ...
            <div class="col-6 col-lg-3">
                <div class="card text-center">
                    <div class="card-body p-t-10">
                        ...
                        <h5 class="mt-3 mb-2">
                            <b>{{ format_price($transactions->where('service', $type)->where('status', 'successful')->sum('amount')) }}</b>
                        </h5>
                        <p class="text-muted mb-0 mt-3">
                            <b>{{ format_price(App\Models\Transaction::whereDate('updated_at', today())->where('service', $type)->where('status', 'successful')->sum('amount')) }}</b>
                            Today
                        </p>
                    </div>
                </div>
                                            ...
                                            <div class="dropdown-menu">
                                                @if ($item->status == 2)
                                                    <a class="dropdown-item"
                                                        href="{{ route('admin.reports.swap.approve', $item->id) }}"
                                                        title="pay">Approve</a>
                                                    <a class="dropdown-item"
                                                        href="{{ route('admin.reports.swap.delete', $item->id) }}"
                                                        title="delete">Reject</a>
                                                @endif
                                            </div>
                                        </div>
                                    </td>
                        ...
                        <tbody>
                            @foreach ($mpayments as $key => $item)
                                <tr>
                                    <td>{{ $key + 1 }}</td>
                                    <td>{{ $item->code }}</td>
                                    <td>{{ $item->user->username ?? 'Deleted' }}</td>
                                    <td>{{ format_price($item->amount) }}</td>
                                    ...
                                    <td>
                                        <div class="dropstart">
                                            ...
                                            <div class="dropdown-menu">
                                                @if ($item->status == 2)
                                                    <a class="dropdown-item"
                                                        href="{{ route('admin.mdeposit.pay', $item->id) }}"
                                                        title="pay">Confirm</a>
                                                    <a class="dropdown-item"
                                                        href="{{ route('admin.mdeposit.reject', $item->id) }}"
                                                        title="delete">Reject</a>
                                                @endif
                                                {{-- <a class="dropdown-item" href="{{route('admin.mdeposit.delete',$item->id)}}" title="delete">Delete </a> --}}
                                                <a class="dropdown-item" href="#" data-bs-toggle="modal"
                                                    data-bs-target="#manualReceipt{{ $item->id }}">View</a>
                                            </div>
                                        </div>
                                    </td>
                                </tr>
                                {{-- Modal --}}
                                <div class="modal fade" id="manualReceipt{{ $item->id }}" tabindex="-1"
